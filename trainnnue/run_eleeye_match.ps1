...
param(
    ...
    [ValidateRange(0.01, 60.0)]
    [double]$EleeyeSecondsPerMove = 0.02,
    [ValidateRange(20, 1000)]
    [int]$MaxPlies = 160,
    [string]$EleeyePath = "E:\ElephantEye\BIN\ELEEYE.EXE",
    [string]$Output = "",
    [switch]$Force
)
...
$Bridge = Join-Path $Root "eleeye_bridge.py"
...
$caseName = "start{0}_pairs{1}_a{2}_b{3}" -f $OpeningStart, $OpeningPairs,
    ...
    ([int][math]::Round($EleeyeSecondsPerMove * 1000))
$WorkDir = Join-Path $PSScriptRoot ("eleeye_match_shards\" + $caseName)
if (-not $Output) {
    $Output = Join-Path $PSScriptRoot ("eleeye_{0}.json" -f $caseName)
...
foreach ($path in @($Engine, $Model, $EleeyePath, $Bridge, $Openings,
                     $MatchScript, $MergeScript)) {
    if (-not (Test-Path -LiteralPath $path)) { throw "missing input: $path" }
}

$env:ELEEYE_PATH = (Resolve-Path -LiteralPath $EleeyePath).Path
...
for ($index = 0; $index -lt $Jobs; $index++) {
    ...
    $arguments = @(
        ...
        "--b-exe", $Python,
        "--b-arg", $Bridge,
        "--b-name", "ElephantEye-3.1",
        "--openings", $Openings,
        "--opening-start", ($OpeningStart + $index * $perJob),
        "--limit", $perJob,
        "--seconds", $EleeyeSecondsPerMove,
        "--a-seconds", $XqSecondsPerMove,
        "--b-seconds", $EleeyeSecondsPerMove,
        "--max-plies", $MaxPlies,
        "--cpu-index", $index,
        "--output", $part
    )
...
if ($processes.Count -gt 0) {
    ...
    if ($failed.Count -gt 0) {
        throw "ElephantEye match shard failure: $($failed.Id -join ',')"
    }
}
...
$result | Add-Member -Force NoteProperty protocol ([ordered]@{
    ...
    threads = 1
    time_control = "fixed time per move; nominal times calibrated on openings 0..11"
    eleeye_version = "ElephantEye 3.1"
    eleeye_protocol = "UCCI 3.0"
    eleeye_book = "disabled"
    eleeye_sha256 = (Get-FileHash $EleeyePath -Algorithm SHA256).Hash
...
Write-Host "ElephantEye match: $Output"

    ...
    int quiescence_search(int alpha, int beta, bool maximizing_player, int qs_depth = 0) {
        ...
        if (in_check) {
            ...
            for (int i = 0; i < rn; ++i) {
                ...
                if (vv >= av) moves[nm++] = m;
#ifdef NNUE_TEACHER
                // The label teacher searches every capture.  SEE-based
                // qsearch filtering is useful in play, but it is selective
                // and can turn a tactical mistake into a training target.
                else moves[nm++] = m;
#else

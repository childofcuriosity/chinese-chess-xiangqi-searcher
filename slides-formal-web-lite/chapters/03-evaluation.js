...
量化价值模型 vθ(x) + 原PST`,
      ...
      },
      {
        ...
        takeaway: 'NNUE不只“训练完成”：量化部署后在公平直接对局中把PST得分率从50%基准提升到59.77%。'
      },
      {
        id: 'e18a',
      ...
      },
      {
        ...
        table: {
          ...
          rows: [
            ...
            ['自有NNUE', '百万D3/D4数据训练，增量推理与量化部署已经完成', '网页默认选项；最佳版本对PST得分率59.77%']
          ]
        },
        steps: ['关系项增加信息，却让实际运行慢了好几倍，因此移除。', 'PST继续承担可解释基线，便于定位网络究竟带来了多少收益。', 'NNUE在同搜索、同CPU时间的直接对局中超过PST，成为产品默认选项。'],
        notes: '把这一章的评价路线放在一起看。仅计算子力时，只看棋子种类与数量，是最简单的基线；PST继续保持O(1)增量更新，又加入位置信息，速度快、行为透明，因此没有因为NNUE完成就被删除。\n\n[按键] 我实际加入过棋子关系项，它增加了局面信息，却让程序慢了好几倍。评价与搜索共享同一份时间预算，所以这条路线被移除。这个失败尝试也留下了一个工程原则：评价不只要更准，还必须让引擎在固定时间内搜得动。\n\n[按键] 自有NNUE随后完成了数据生成、训练、整数化、增量累加器、网页部署和公平对局。最佳D4-H16模型对PST得分率59.77%，相对等强基准高9.77个百分点；但平均搜索深度少约0.46层。于是当前产品默认NNUE，同时保留PST作为对照和低成本选择。仓库中的Pikafish仍是第三方开源引擎，只用于更强对照与协议桥接。',

#!/usr/bin/env python3
"""Expose Xiangqi Tianqi 1.1.8 (UCI) through the project's line protocol."""
...
class Bridge:
    def __init__(self) -> None:
        ...
        configured = os.environ.get("TIANQI_PATH")
        if not configured:
            raise RuntimeError("TIANQI_PATH is not set")
        executable = Path(configured).resolve()
        if not executable.is_file():
            raise FileNotFoundError(f"Tianqi executable not found: {executable}")
        ...
        self.movetime_ms = 100
        self._initialize()

    def send(self, line: str) -> None:
        if self.process.poll() is not None:
            raise RuntimeError("Tianqi process has exited")
    ...
    def wait_for(self, prefix: str) -> str:
        ...
        while True:
            ...
            if not line:
                raise RuntimeError(f"Tianqi exited while waiting for {prefix}")
            line = line.strip()
    ...
    def search(self) -> None:
        ...
        while True:
            line = self.process.stdout.readline()
            if not line:
                raise RuntimeError("Tianqi exited while searching")
...
def main() -> int:
    ...
    try:
        ...
        print(f"Tianqi bridge error: {exc}", file=sys.stderr, flush=True)
        print("resign", flush=True)
        return 1
    finally:
